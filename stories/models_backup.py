...
class StoryView(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['story', '-last_viewed']),
            models.Index(fields=['user', '-last_viewed']),
        ]
    
    def __str__(self):
        return f'{self.story.title} - {self.user or self.ip_address}'
...
class Playlist(models.Model):
    ...
    @property
    def calculated_stories_count(self):
        """Алиас для stories_count для совместимости с аннотациями"""
        return getattr(self, '_calculated_stories_count', self.stories_count)
